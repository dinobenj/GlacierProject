...
area_chart <- function(glacier_name) {
  ...
  p <- plot(plot_data$YEAR, 
            plot_data$AREA_CHANGE, 
            main = "Deez Nut", 
            xlab = "Year", 
            ylab = "Area Change", 
            xlim = c(1950, 2020), 
  ...
  b <- barplot(plot_data$AREA_CHANGE, 
...
display_raster <- function(p) {
  ...
  elevation <- get_elev_raster(location = tmp, 
                               prj = "EPSG:4326", z = 9)
  pl <- plot(elevation, 
             main = p$id, 
             xlab = "Longitude", 
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    menuItem(
            selectInput(inputId = "Input_Country_Code", 
                         label = "Select 2 Letter Country Code", 
                         selected = TRUE, 
                         multiple = FALSE, 
                         choices = sort(dd$POLITICAL_UNIT))),
    selectInput(inputId = "Input_Glacier_Name", 
                label = "Select Glacier:", 
                multiple = FALSE, 
                choices = sort(dd$NAME)),
